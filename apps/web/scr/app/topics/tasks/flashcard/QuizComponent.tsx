...
import { generateQuizQuestions, generateLocalQuizQuestions, QuizQuestion } from './quizService';
  ...
  useEffect(() => {
    const fetchQuizQuestions = async () => {
      ...
      try {
        ...
        try {
          const questions = await generateQuizQuestions(topic, flashcards);
          setQuizQuestions(questions);
          setIsLoading(false);
        } catch (error) {
          console.error('Error generating quiz from API:', error);
          // Fall back to local generation if OpenAI fails
          console.log('Falling back to locally generated quiz questions');
          const localQuestions = generateLocalQuizQuestions(flashcards);
